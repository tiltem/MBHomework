...
@implementation ArrayOfProgram

@synthesize items;

+ (ArrayOfProgram *)createWithXml:(DDXMLElement *)__node __request:(RequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(RequestResultHandler*) __request {
    if(self = [self init])
    {
        NSArray* tempItems = [__node elementsForName:@"Program"];
        for(DDXMLElement* child in tempItems)
        {
            [self.items addObject:(Program*)[__request createObject:child type:[Program class]]];
        }
...
{
    for (Program* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[Program class] name:@"Program" URI:@"http://clients.mindbodyonline.com/api/0_5" parent:__parent skipNullProperty:__request.SkipNullProperties];
...
- (Program*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(Program*)anObject {
...
- (BOOL)containsObject:(Program*)anObject {
...
- (NSUInteger)indexOfObject:(Program*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(Program*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Program*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Program*)anObject inRange:(NSRange)range {
...
- (void)addObject:(Program*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(Program*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(Program*)anObject {
...
- (void)removeObject:(Program*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(Program*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(Program*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(Program*)anObject {
...
+ (id)array {
	return [[ ArrayOfProgram alloc] init];
}

+ (id)arrayWithObject:(Program*)anObject {
	return [[ ArrayOfProgram alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const Program* *)objects count:(NSUInteger)cnt {
	return [[ ArrayOfProgram alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(Program*)firstObj, ... {
	ArrayOfProgram* a = [ ArrayOfProgram array];
	id eachObject;
...
- (id)initWithObjects:(Program*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ ArrayOfProgram alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ ArrayOfProgram alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ ArrayOfProgram alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ ArrayOfProgram alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const Program* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Program*, Program*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Program*, Program*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(Program*, Program*, void *))compare context:(void *)context {
