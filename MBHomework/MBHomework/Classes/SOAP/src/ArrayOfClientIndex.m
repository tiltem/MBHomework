...
@implementation ArrayOfClientIndex

@synthesize items;

+ (ArrayOfClientIndex *)createWithXml:(DDXMLElement *)__node __request:(RequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(RequestResultHandler*) __request {
    if(self = [self init])
    {
        NSArray* tempItems = [__node elementsForName:@"ClientIndex"];
        for(DDXMLElement* child in tempItems)
        {
            [self.items addObject:(ClientIndex*)[__request createObject:child type:[ClientIndex class]]];
        }
...
{
    for (ClientIndex* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[ClientIndex class] name:@"ClientIndex" URI:@"http://clients.mindbodyonline.com/api/0_5" parent:__parent skipNullProperty:__request.SkipNullProperties];
...
- (ClientIndex*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(ClientIndex*)anObject {
...
- (BOOL)containsObject:(ClientIndex*)anObject {
...
- (NSUInteger)indexOfObject:(ClientIndex*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(ClientIndex*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(ClientIndex*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(ClientIndex*)anObject inRange:(NSRange)range {
...
- (void)addObject:(ClientIndex*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(ClientIndex*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(ClientIndex*)anObject {
...
- (void)removeObject:(ClientIndex*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(ClientIndex*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(ClientIndex*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(ClientIndex*)anObject {
...
+ (id)array {
	return [[ ArrayOfClientIndex alloc] init];
}

+ (id)arrayWithObject:(ClientIndex*)anObject {
	return [[ ArrayOfClientIndex alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const ClientIndex* *)objects count:(NSUInteger)cnt {
	return [[ ArrayOfClientIndex alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(ClientIndex*)firstObj, ... {
	ArrayOfClientIndex* a = [ ArrayOfClientIndex array];
	id eachObject;
...
- (id)initWithObjects:(ClientIndex*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ ArrayOfClientIndex alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ ArrayOfClientIndex alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ ArrayOfClientIndex alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ ArrayOfClientIndex alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const ClientIndex* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(ClientIndex*, ClientIndex*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(ClientIndex*, ClientIndex*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(ClientIndex*, ClientIndex*, void *))compare context:(void *)context {
