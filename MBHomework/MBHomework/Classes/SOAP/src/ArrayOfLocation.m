...
@implementation ArrayOfLocation

@synthesize items;

+ (ArrayOfLocation *)createWithXml:(DDXMLElement *)__node __request:(RequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(RequestResultHandler*) __request {
    if(self = [self init])
    {
        NSArray* tempItems = [__node elementsForName:@"Location"];
        for(DDXMLElement* child in tempItems)
        {
            [self.items addObject:(Location*)[__request createObject:child type:[Location class]]];
        }
...
-(void) serialize:(DDXMLElement*)__parent __request:(RequestResultHandler*) __request
{
    for (Location* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[Location class] name:@"Location" URI:@"http://clients.mindbodyonline.com/api/0_5" parent:__parent skipNullProperty:__request.SkipNullProperties];
...
- (Location*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(Location*)anObject {
...
- (BOOL)containsObject:(Location*)anObject {
...
- (NSUInteger)indexOfObject:(Location*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(Location*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Location*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Location*)anObject inRange:(NSRange)range {
...
- (void)addObject:(Location*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(Location*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(Location*)anObject {
...
- (void)removeObject:(Location*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(Location*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(Location*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(Location*)anObject {
...
+ (id)array {
	return [[ ArrayOfLocation alloc] init];
}

+ (id)arrayWithObject:(Location*)anObject {
	return [[ ArrayOfLocation alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const Location* *)objects count:(NSUInteger)cnt {
	return [[ ArrayOfLocation alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(Location*)firstObj, ... {
	ArrayOfLocation* a = [ ArrayOfLocation array];
	id eachObject;
...
- (id)initWithObjects:(Location*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ ArrayOfLocation alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ ArrayOfLocation alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ ArrayOfLocation alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ ArrayOfLocation alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const Location* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Location*, Location*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Location*, Location*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(Location*, Location*, void *))compare context:(void *)context {
