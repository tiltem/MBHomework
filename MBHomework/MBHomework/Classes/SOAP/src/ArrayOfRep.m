...
@implementation ArrayOfRep

@synthesize items;

+ (ArrayOfRep *)createWithXml:(DDXMLElement *)__node __request:(RequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(RequestResultHandler*) __request {
    if(self = [self init])
    {
        NSArray* tempItems = [__node elementsForName:@"Rep"];
        for(DDXMLElement* child in tempItems)
        {
            [self.items addObject:(Rep*)[__request createObject:child type:[Rep class]]];
        }
...
{
    for (Rep* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[Rep class] name:@"Rep" URI:@"http://clients.mindbodyonline.com/api/0_5" parent:__parent skipNullProperty:__request.SkipNullProperties];
...
- (Rep*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(Rep*)anObject {
...
- (BOOL)containsObject:(Rep*)anObject {
...
- (NSUInteger)indexOfObject:(Rep*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(Rep*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Rep*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Rep*)anObject inRange:(NSRange)range {
...
- (void)addObject:(Rep*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(Rep*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(Rep*)anObject {
...
- (void)removeObject:(Rep*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(Rep*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(Rep*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(Rep*)anObject {
...
+ (id)array {
	return [[ ArrayOfRep alloc] init];
}

+ (id)arrayWithObject:(Rep*)anObject {
	return [[ ArrayOfRep alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const Rep* *)objects count:(NSUInteger)cnt {
	return [[ ArrayOfRep alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(Rep*)firstObj, ... {
	ArrayOfRep* a = [ ArrayOfRep array];
	id eachObject;
...
- (id)initWithObjects:(Rep*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ ArrayOfRep alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ ArrayOfRep alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ ArrayOfRep alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ ArrayOfRep alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const Rep* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Rep*, Rep*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Rep*, Rep*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(Rep*, Rep*, void *))compare context:(void *)context {
