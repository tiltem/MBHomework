...
@implementation ArrayOfAvailability

@synthesize items;

+ (ArrayOfAvailability *)createWithXml:(DDXMLElement *)__node __request:(RequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(RequestResultHandler*) __request {
    if(self = [self init])
    {
        NSArray* tempItems = [__node elementsForName:@"Availability"];
        for(DDXMLElement* child in tempItems)
        {
            [self.items addObject:(Availability*)[__request createObject:child type:[Availability class]]];
        }
...
{
    for (Availability* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[Availability class] name:@"Availability" URI:@"http://clients.mindbodyonline.com/api/0_5" parent:__parent skipNullProperty:__request.SkipNullProperties];
...
- (Availability*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(Availability*)anObject {
...
- (BOOL)containsObject:(Availability*)anObject {
...
- (NSUInteger)indexOfObject:(Availability*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(Availability*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Availability*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Availability*)anObject inRange:(NSRange)range {
...
- (void)addObject:(Availability*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(Availability*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(Availability*)anObject {
...
- (void)removeObject:(Availability*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(Availability*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(Availability*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(Availability*)anObject {
...
+ (id)array {
	return [[ ArrayOfAvailability alloc] init];
}

+ (id)arrayWithObject:(Availability*)anObject {
	return [[ ArrayOfAvailability alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const Availability* *)objects count:(NSUInteger)cnt {
	return [[ ArrayOfAvailability alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(Availability*)firstObj, ... {
	ArrayOfAvailability* a = [ ArrayOfAvailability array];
	id eachObject;
...
- (id)initWithObjects:(Availability*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ ArrayOfAvailability alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ ArrayOfAvailability alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ ArrayOfAvailability alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ ArrayOfAvailability alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const Availability* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Availability*, Availability*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Availability*, Availability*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(Availability*, Availability*, void *))compare context:(void *)context {
