...
#import "Option.h"
#import "Helper.h"
#import "ArrayOfOption.h"


@implementation ArrayOfOption

@synthesize items;

+ (ArrayOfOption *)createWithXml:(DDXMLElement *)__node __request:(RequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(RequestResultHandler*) __request {
    if(self = [self init])
    {
        NSArray* tempItems = [__node elementsForName:@"Option"];
        for(DDXMLElement* child in tempItems)
        {
            [self.items addObject:(Option*)[__request createObject:child type:[Option class]]];
        }
...
{
    for (Option* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[Option class] name:@"Option" URI:@"http://clients.mindbodyonline.com/api/0_5" parent:__parent skipNullProperty:__request.SkipNullProperties];
...
- (Option*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(Option*)anObject {
...
- (BOOL)containsObject:(Option*)anObject {
...
- (NSUInteger)indexOfObject:(Option*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(Option*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Option*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Option*)anObject inRange:(NSRange)range {
...
- (void)addObject:(Option*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(Option*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(Option*)anObject {
...
- (void)removeObject:(Option*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(Option*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(Option*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(Option*)anObject {
...
+ (id)array {
	return [[ ArrayOfOption alloc] init];
}

+ (id)arrayWithObject:(Option*)anObject {
	return [[ ArrayOfOption alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const Option* *)objects count:(NSUInteger)cnt {
	return [[ ArrayOfOption alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(Option*)firstObj, ... {
	ArrayOfOption* a = [ ArrayOfOption array];
	id eachObject;
...
- (id)initWithObjects:(Option*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ ArrayOfOption alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ ArrayOfOption alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ ArrayOfOption alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ ArrayOfOption alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const Option* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Option*, Option*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Option*, Option*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(Option*, Option*, void *))compare context:(void *)context {
