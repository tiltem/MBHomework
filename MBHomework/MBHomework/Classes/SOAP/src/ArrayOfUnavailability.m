...
@implementation ArrayOfUnavailability

@synthesize items;

+ (ArrayOfUnavailability *)createWithXml:(DDXMLElement *)__node __request:(RequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(RequestResultHandler*) __request {
    if(self = [self init])
    {
        NSArray* tempItems = [__node elementsForName:@"Unavailability"];
        for(DDXMLElement* child in tempItems)
        {
            [self.items addObject:(Unavailability*)[__request createObject:child type:[Unavailability class]]];
        }
...
{
    for (Unavailability* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[Unavailability class] name:@"Unavailability" URI:@"http://clients.mindbodyonline.com/api/0_5" parent:__parent skipNullProperty:__request.SkipNullProperties];
...
- (Unavailability*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(Unavailability*)anObject {
...
- (BOOL)containsObject:(Unavailability*)anObject {
...
- (NSUInteger)indexOfObject:(Unavailability*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(Unavailability*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Unavailability*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(Unavailability*)anObject inRange:(NSRange)range {
...
- (void)addObject:(Unavailability*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(Unavailability*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(Unavailability*)anObject {
...
- (void)removeObject:(Unavailability*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(Unavailability*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(Unavailability*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(Unavailability*)anObject {
...
+ (id)array {
	return [[ ArrayOfUnavailability alloc] init];
}

+ (id)arrayWithObject:(Unavailability*)anObject {
	return [[ ArrayOfUnavailability alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const Unavailability* *)objects count:(NSUInteger)cnt {
	return [[ ArrayOfUnavailability alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(Unavailability*)firstObj, ... {
	ArrayOfUnavailability* a = [ ArrayOfUnavailability array];
	id eachObject;
...
- (id)initWithObjects:(Unavailability*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ ArrayOfUnavailability alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ ArrayOfUnavailability alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ ArrayOfUnavailability alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ ArrayOfUnavailability alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const Unavailability* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Unavailability*, Unavailability*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(Unavailability*, Unavailability*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(Unavailability*, Unavailability*, void *))compare context:(void *)context {
